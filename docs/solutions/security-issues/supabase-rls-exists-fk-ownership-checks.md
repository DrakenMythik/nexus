...
**`user_program_enrollments` - published program on INSERT/UPDATE:**
...
**Verified:** `npm run db:reset`, `npm run db:lint`.

## Why This Works

RLS `WITH CHECK` runs on the **new row image** for INSERT and on the **updated row** for UPDATE. `EXISTS` subqueries tie each FK column to an authoritative row the caller is allowed to use: `workout_sessions.user_id` for session scope, `programs.is_published` for enrollments and for catalog exercises via `program_exercises -> program_days -> programs`. An attacker cannot satisfy the policy with another user session ID or an unpublished program/exercise while keeping `user_id = auth.uid()`. Nullable `program_exercise_id` is explicitly allowed when null; otherwise the join enforces the same publish gate as catalog SELECT (aligned with U1-KTD-2 in the U1 plan).
...
6. **Name policies honestly** - `*_own` should mean row owner and allowed FK targets.
7. **Threat-model INSERT and UPDATE separately** (reassignment on UPDATE is easy to miss).
8. **Run `npm run db:reset` and `npm run db:lint`** after migration edits; spot-check with a second-user JWT per plan AE2.
9. **Blueprint first** per `AGENTS.md`: plan/ERD must call out FK-aware RLS before editing `supabase/migrations/`.
10. **Run `/ce-code-review`** on migration PRs before merge; tiered review surfaced these gaps post-implementation.
